...
contains
  ...
  subroutine get_expVabs
    ...
    if ( trim(jobtype).eq.flag_soc ) then
       call cpu_time(start)
       call Zexp_mat(nstuse,Zabp(1:nstuse*nstuse),Zexp_abp(1:nstuse*nstuse),hdt)
       call cpu_time(finish)
       write(iout,"(' exponential of V_abs time:',f12.4,' s')") finish-start       
    else
       call cpu_time(start)
       call exp_mat(nstuse,abp(1:nstuse*nstuse),exp_abp(1:nstuse*nstuse),hdt)
       call cpu_time(finish)
       write(iout,"(' exponential of V_abs time:',f12.4,' s')") finish-start
    end if
  ...
  subroutine trotter_linear
    ...
    dir_loop : do idir=1, ndir
       ...
       !: hp = W * exp(-Vabs dt/2)
       call matmult( hp1, exp_abp(1:nstuse2), nstuse )
       ...
       emax_loop : do iemax=1, nemax
          ...
          timestep_loop : do itime=1, nstep-1
             ...
             analysis : if ( mod(itime,outstep).eq.0 ) then
                ...
                write( funit2,"( i5,f10.4,1x,2(f10.7,1x),3(f10.7,1x),2(1x,f15.10),3(1x,f15.10) )") &
                     idata, dble(itime)*dt*au2fs, efield1, efield2, &
                     efield1*dirx1+efield2*dirx2, efield1*diry1+efield2*diry2, efield1*dirz1+efield2*dirz2, &
                     norm**2, rate/au2fs, mux, muy, muz
                flush(funit2)
